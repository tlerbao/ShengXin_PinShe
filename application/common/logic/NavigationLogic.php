...
class NavigationLogic extends Model
{
        ...
        foreach ($oneLevel as $yik => $yiv) {
            ...
            foreach ($twoLevel as $key => $val) {
                if ($yiv['id'] == $val['parent_id']) {
                    ...
                    //二级下拉框
                    $HtmlCode .= '<option value="' . $val['id'] . '" data-typeurl="' . $typeurl . '" data-typename="' . $val['typename'] . '" ' . $style1 . '>&nbsp; &nbsp;' . $val['typename'] . '</option>';
                    foreach ($threeLevel as $k => $v) {
                        if ($val['id'] == $v['parent_id']) {
                            ...
                            //三级下拉框
                            $HtmlCode .= '<option value="' . $v['id'] . '" data-typeurl="' . $typeurl . '" data-typename="' . $v['typename'] . '"' . $style1 . '>&nbsp; &nbsp; &nbsp; &nbsp;' . $v['typename'] . '</option>';
                            unset($threeLevel[$k]);
                        }
                    }
                    unset($twoLevel[$key]);
    ...
    public function GetAllNav($position_id = 0, $nav_id = 0)
    {
        ...
        $ids         = convert_arr_key($ArcTypeData, 'nav_id');
